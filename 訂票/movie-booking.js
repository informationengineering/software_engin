...
const MAX_MEALS = 10;
const MOVIES_API = 'http://localhost:3001/api/movies/now-showing';
const SHOWTIMES_API = 'http://localhost:3001/api/showtimes';
const TICKET_CLASSES_API = 'http://localhost:3001/api/ticket/classes'; 
const SEAT_CONDITION_API = 'http://localhost:3001/api/seats/condition/'; 
const MEAL_MENU_API = 'http://localhost:3001/api/meals/menu'; 
const CHECK_BALANCE_API = 'http://localhost:3001/api/member/check-balance/';
const FINAL_CHECKOUT_API = 'http://localhost:3001/api/booking/create';
const LOGIN_API = 'http://localhost:3001/api/member/login';
...
// --------------------------------------------------------------------------------
function MealSelection({ onNext, ticketCount = 2 }) {
    ...
    React.useEffect(() => {
        const loadMealMenu = async () => {
            try {
                ...
                setError(`連線失敗！請確認後端 ${MEAL_MENU_API} 服務是否已啟動。`);
                setIsLoading(false);
            }
        };
        loadMealMenu();
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            if (response.data.success && response.data.memberID) {
                alert(`登入成功！會員ID: ${response.data.memberID}`);
...
function PaymentSelection({ bookingData, memberId, onCheckoutSuccess }) {
    ...
    const handleFinalCheckout = async () => {
        ...
        try {
            const orderPayload = {
                memberID: memberId,
                showingID: bookingData.showtime.showingId,
...
function BookingFlow() {
    ...
    const handleLoginSuccess = (id) => {
        setMemberId(id);
        setCurrentStep('payment'); // 登入成功，繼續到付款頁面
    };
    ...
    if (currentStep === 'showtime') { 
        ...
        ComponentToRender = <TicketSelection onNext={handleNextStep} />;
        ...
        ComponentToRender = <MealSelection onNext={handleNextStep} ticketCount={ticketCount} />;
